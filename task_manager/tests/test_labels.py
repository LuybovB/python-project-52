...
class StatusTests(TestCase):
    def setUp(self):
        ...
        self.status2 = Status.objects.create(name='AnotherStatus')
    ...
    def test_update_status_without_login(self):
        update_status_url = (
            reverse('update_status', kwargs={'pk': self.status.pk}))
        login_url = reverse('login')
        expected_redirect_url = f"{login_url}?next={update_status_url}"
        response = self.client.get(update_status_url)
    ...
    def test_delete_status_without_login(self):
        ...
        login_url = reverse('login')
        expected_redirect_url = f"{login_url}?next={delete_url}"
    ...
    def test_delete_status_with_login(self):
        ...
        # Создаем статус, который будем удалять.
        status_to_delete = Status.objects.create(name='ToBeDeleted')

        response = self.client.post(
            reverse('delete_status',
                    kwargs={'pk': status_to_delete.pk}), follow=True)
        self.assertRedirects(response, reverse('list_statuses'))
        ...
        self.assertFalse(
            Status.objects.filter(pk=status_to_delete.pk).exists())

    def test_delete_status_in_use(self):

        self.client.login(username='testuser', password='testpassword123')

        # Создание задачи, связанной с данным статусом.
        Task.objects.create(
            name='Test Task',
            status=self.status,
            author=self.user,
        ...
        response = self.client.post(
            reverse('delete_status',
                    kwargs={'pk': self.status.pk}), follow=True)
        self.assertRedirects(response, reverse('list_statuses'))
    ...
    def test_list_statuses_view_with_login(self):
        self.client.login(
            username='testuser',
